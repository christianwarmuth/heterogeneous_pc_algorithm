...
option_list = list(
    ...
    make_option(c("-f", "--split_factor"), type = "double", default = 0.01,
        help = "split factor for heterogeneous execution [split_factor = 1.0]", metavar = "split_factor"),
    make_option(c("-m", "--managed_memory"), type = "integer", default = 1,
        help = "managed memory for heterogeneous execution [managed_memory = 1]", metavar = "managed_memory"),
    make_option(c("-o", "--output_file"), type = "character", default = "",
        help = "output file for storing the algorithm runtimes [output_file = '']", metavar = "output_file"))
...
uB <- opt$upper_bound
alpha <- opt$alpha
splitFactor <- opt$split_factor
managedMemory <- opt$managed_memory
...
row <- list(p, n, prob, lB, uB, alpha, splitFactor, managedMemory)

write.table(row, file = outputFile, sep = ",", append = TRUE, quote = FALSE, col.names = FALSE, 
    row.names = FALSE, eol = ",")

cuPC_fit <- cu_pc(suffStat, p = p, alpha = alpha, splitFactor = splitFactor, managedMemory = managedMemory, 
