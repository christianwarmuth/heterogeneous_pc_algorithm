...
void Skeleton(double* C, int* P, int* G, double* Th, int* l, int* maxlevel, double* pMax, int* SepSet, char** o) {
  ...
  *l = 0;
  ...
  cudaMalloc((void**)&counters, num_levels * sizeof(unsigned long long));
  cudaMemset(counters, 0, num_levels * sizeof(unsigned long long));
  ...
  for (*l = 0; *l <= ML && !FinishFlag && *l <= *maxlevel; *l = *l + 1) {
    if (*l == 0) {
      if ((n * n) < 1024) {
        ...
        cal_Indepl0<<<BLOCKS_PER_GRID, THREADS_PER_BLOCK>>>(C_cuda, G_cuda, Th[0], pMax_cuda, n, counters);
        cudaDeviceSynchronize();
      } else {
      ...
      if (*l == 1) {
        ...
        // cudaMalloc((void**)&SepSet_cuda,  n * n * 1 * sizeof(int));
        cal_Indepl1<<<BLOCKS_PER_GRID, THREADS_PER_BLOCK, nprime * sizeof(int)>>>(
            C_cuda, G_cuda, GPrime_cuda, mutex_cuda, SepSet_cuda, pMax_cuda, Th[1], n, counters);
        ...
        cal_Indepl2<<<BLOCKS_PER_GRID, THREADS_PER_BLOCK, nprime * sizeof(int)>>>(
            C_cuda, G_cuda, GPrime_cuda, mutex_cuda, SepSet_cuda, pMax_cuda, n, Th[2], counters);
        // cudaFree(SepSet_cuda);
        cudaDeviceSynchronize();
      } else if (*l == 3) {
        ...
        cal_Indepl3<<<BLOCKS_PER_GRID, THREADS_PER_BLOCK, nprime * sizeof(int)>>>(
            C_cuda, G_cuda, GPrime_cuda, mutex_cuda, SepSet_cuda, pMax_cuda, n, Th[3], counters);
        // cudaFree(SepSet_cuda);
        cudaDeviceSynchronize();
      } else if (*l == 4) {
        ...
        cal_Indepl4<<<BLOCKS_PER_GRID, THREADS_PER_BLOCK, nprime * sizeof(int)>>>(
            C_cuda, G_cuda, GPrime_cuda, mutex_cuda, SepSet_cuda, pMax_cuda, n, Th[4], counters);
        cudaDeviceSynchronize();
      } else if (*l == 5) {
        ...
        cal_Indepl5<<<BLOCKS_PER_GRID, THREADS_PER_BLOCK, nprime * sizeof(int)>>>(
            C_cuda, G_cuda, GPrime_cuda, mutex_cuda, SepSet_cuda, pMax_cuda, n, Th[5], counters);
        cudaDeviceSynchronize();
      } else if (*l == 6) {
        ...
        cal_Indepl6<<<BLOCKS_PER_GRID, THREADS_PER_BLOCK, nprime * sizeof(int)>>>(
            C_cuda, G_cuda, GPrime_cuda, mutex_cuda, SepSet_cuda, pMax_cuda, n, Th[6], counters);
        cudaDeviceSynchronize();
      } else if (*l == 7) {
        ...
        cal_Indepl7<<<BLOCKS_PER_GRID, THREADS_PER_BLOCK, nprime * sizeof(int)>>>(
            C_cuda, G_cuda, GPrime_cuda, mutex_cuda, SepSet_cuda, pMax_cuda, n, Th[7], counters);
        cudaDeviceSynchronize();
      } else if (*l == 8) {
        ...
        cal_Indepl8<<<BLOCKS_PER_GRID, THREADS_PER_BLOCK, nprime * sizeof(int)>>>(
            C_cuda, G_cuda, GPrime_cuda, mutex_cuda, SepSet_cuda, pMax_cuda, n, Th[8], counters);
        cudaDeviceSynchronize();
      } else if (*l == 9) {
        ...
        cal_Indepl9<<<BLOCKS_PER_GRID, THREADS_PER_BLOCK, nprime * sizeof(int)>>>(
            C_cuda, G_cuda, GPrime_cuda, mutex_cuda, SepSet_cuda, pMax_cuda, n, Th[9], counters);
        cudaDeviceSynchronize();
      } else if (*l == 10) {
        ...
        cal_Indepl10<<<BLOCKS_PER_GRID, THREADS_PER_BLOCK, nprime * sizeof(int)>>>(
            C_cuda, G_cuda, GPrime_cuda, mutex_cuda, SepSet_cuda, pMax_cuda, n, Th[10], counters);
        cudaDeviceSynchronize();
      } else if (*l == 11) {
        ...
        cal_Indepl11<<<BLOCKS_PER_GRID, THREADS_PER_BLOCK, nprime * sizeof(int)>>>(
            C_cuda, G_cuda, GPrime_cuda, mutex_cuda, SepSet_cuda, pMax_cuda, n, Th[11], counters);
        cudaDeviceSynchronize();
      } else if (*l == 12) {
        ...
        cal_Indepl12<<<BLOCKS_PER_GRID, THREADS_PER_BLOCK, nprime * sizeof(int)>>>(
            C_cuda, G_cuda, GPrime_cuda, mutex_cuda, SepSet_cuda, pMax_cuda, n, Th[12], counters);
        cudaDeviceSynchronize();
      } else if (*l == 13) {
        ...
        cal_Indepl13<<<BLOCKS_PER_GRID, THREADS_PER_BLOCK, nprime * sizeof(int)>>>(
            C_cuda, G_cuda, GPrime_cuda, mutex_cuda, SepSet_cuda, pMax_cuda, n, Th[13], counters);
        cudaDeviceSynchronize();
      } else if (*l == 14) {
        ...
        cal_Indepl14<<<BLOCKS_PER_GRID, THREADS_PER_BLOCK, nprime * sizeof(int)>>>(
            C_cuda, G_cuda, GPrime_cuda, mutex_cuda, SepSet_cuda, pMax_cuda, n, Th[14], counters);
        cudaDeviceSynchronize();
      } else {
        // TODO: add PC serial
      }
  ...
  cudaMemcpy(SepSet, SepSet_cuda, n * n * ML * sizeof(int), cudaMemcpyDeviceToHost);
  // Copy  Pmax from GPU to CPU
  cudaMemcpy(pMax, pMax_cuda, n * n * sizeof(double), cudaMemcpyDeviceToHost);
  // Preprocess pMax
  double temp = 0;
  for (int i = 0; i < n; i++) {
    pMax[i * n + i] = 1;
  ...
  unsigned long long* cpu_counters = new unsigned long long[num_levels];
  cudaMemcpy(cpu_counters, counters, num_levels * sizeof(unsigned long long), cudaMemcpyDeviceToHost);

  std::ofstream output_stream(o[0], std::ofstream::app);

  if (output_stream) {
    for (int i = 0; i < num_levels; ++i) {
      output_stream << cpu_counters[i] << (i < num_levels - 1 ? "," : "\n");
    }

    output_stream.close();
  } else {
    for (int i = 0; i < num_levels; ++i) {
      std::cout << cpu_counters[i] << (i < num_levels - 1 ? "," : "\n");
    }
  }
...
__global__ void cal_Indepl0(double* C, int* G, double th, double* pMax, int n, unsigned long long* counters) {
